...
namespace App\Livewire\Assistant\Attendance;
...
class Overview extends Component
{
    ...
    public function loadOverviewData()
    {
        ...
        $totalStudents = User::whereHas('classrooms', function ($query) use ($assistantClassrooms) {
            $query->whereIn('class_id', $assistantClassrooms);
        })->where('role', 'student')->count();

        $totalClasses = $assistantClassrooms->count();

        // Thống kê điểm danh trong tháng cho các lớp của assistant
        ...
        // Lấy top 5 lớp có điểm danh nhiều nhất (chỉ các lớp của assistant)
        ...
        // Lấy điểm danh gần đây (chỉ các lớp của assistant)
        $recentAttendances = Attendance::whereIn('class_id', $assistantClassrooms)
    ...
    {
        return view('assistant.attendance.overview');
    }
}
