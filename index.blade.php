                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @forelse ($students as $student)
                                    @php
                                        $avg = $this->calculateAverage($student->id);
                                        $rankData = $this->calculateRank($student->id);
                                    @endphp
                                    <tr>
                                    <td>{{ $student->name }}</td>
                                    <td>
                                    ...
                                    <td>
                                        @if($avg > 0)
                                            <span class="fw-bold {{ $avg >= 8 ? 'text-success' : ($avg >= 5 ? 'text-warning' : 'text-danger') }}">
                                                {{ number_format($avg, 2) }}
                                            </span>
                                        @else
                                    ...
                                    <td>
                                        @if($avg > 0)
                                            <span class="badge bg-secondary">{{ $rankData['rank'] }}/{{ $rankData['total'] }}</span>
                                        @else
                                            <span class="text-muted">—</span>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('assistant.grade-entry-assistant.show', $student->id) }}" class="btn btn-sm btn-info">Xem chi tiết</a>
                                    </td>
                                </tr>
                                @empty
                                <tr>
